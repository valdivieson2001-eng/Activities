...
presence.on('UpdateData', async () => {
  ...
  if (['playing', 'paused'].includes(mediaData.playbackState)) {
    ...
    presenceData = {
      type: ActivityType.Listening,
      largeImageKey: ActivityAssets.Logo,
      details: `🎧 Escuchando ${mediaData.title}`,
      state: `de ${mediaData.artist}`,
      startTimestamp: state.mediaTimestamps[0],
      endTimestamp: state.mediaTimestamps[1],
      buttons: [
        { label: 'Abrir en YouTube Music', url: `https://music.youtube.com/watch?v=${watchID}` },
      ],
    }
